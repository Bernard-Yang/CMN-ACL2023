...
class VAE(nn.Module):
    ...
    def __init__(self, encoder, decoder, tokenizer_encoder, tokenizer_decoder, args, NSP_model=None): #
        ...
        if self.args.fb_mode == 0 or self.args.fb_mode == 3 or self.args.fb_mode == 2:
            self.cls = nn.Linear(768, 2)
            # self.posterior_linear = nn.Linear(768, 2 * self.nz, bias=False)
            # self.neg_posterior_linear = nn.Linear(768, 2 * self.nz, bias=False)
            self.NSP_model = NSP_model
            # self.neg_linear = nn.Linear(2 * 768, 2 * 32, bias=False)
    ...
    def forward(self, inputs, labels, conditions, senetence_b=None, NSP_encoder=None, label=None, segments_tensor=None, mask_tensor=None):
        # (inputs, labels, conditions)
        # pdb.set_trace()   
        ...
        if self.args.fb_mode==0:
            ...
            condition_outputs = self.NSP_model(conditions, condition_attention_mask)
            condition_pooled_hidden_fea = condition_outputs[1]

            # Connect hidden feature to the latent space
            # print(condition_pooled_hidden_fea.shape, raw_pooled_hidden_fea.shape)
            bert_outputs = self.encoder(NSP_encoder, mask_tensor, segments_tensor)
            bert_pooled_hidden_fea = bert_outputs[1]

            seq_relationship_score = self.cls(bert_pooled_hidden_fea)
            if label != None:
                loss_fct = CrossEntropyLoss(ignore_index=-1)
                NSP_loss = loss_fct(seq_relationship_score.view(-1, 2), label.view(-1))
            # # outputs = (next_sentence_loss,) + outputs
            #  = outputs[0]
            senetence_b_attention_mask = (senetence_b > 0).float()
            senetence_b_outputs = self.encoder(senetence_b, senetence_b_attention_mask)
            senetence_b_pooled_hidden_fea = senetence_b_outputs[1]
            if int(label.item()) == 1:
                # c_x = torch.cat([condition_pooled_hidden_fea, raw_pooled_hidden_fea], 1)
                posterior_mu, posterior_logvar = self.encoder.linear(bert_pooled_hidden_fea).chunk(2, -1)
                # print(1, posterior_mu.shape)
            # else:
            #     # c_x = torch.cat([condition_pooled_hidden_fea, senetence_b_pooled_hidden_fea], 1)
            #     posterior_mu, posterior_logvar = self.posterior_linear(bert_pooled_hidden_fea).chunk(2, -1)
            #     # print(0, posterior_mu.shape)
            # print(c_x.shape)
            prior_mu, prior_logvar = self.NSP_model.linear(condition_pooled_hidden_fea).chunk(2, -1)
            if int(label.item()) == 1:
                loss_kl = 1 / 2 * prior_logvar - 1 / 2 * posterior_logvar + (
                            torch.exp(posterior_logvar) + (posterior_mu - prior_mu).pow(2)) / (
                                          2 * torch.exp(prior_logvar)) - 0.5
            # if int(label.item()) == 1:
                latent_z = self.reparameterize(posterior_mu, posterior_logvar, nsamples=1)

                latent_z = latent_z.squeeze(1)
                kl_mask = (loss_kl > self.args.dim_target_kl).float()
                # kl_mask = (loss_kl > 0).float()
                loss_kl = (kl_mask * loss_kl).sum(dim=1)
            else:
                latent_z = self.reparameterize(prior_mu, prior_logvar, nsamples=1)
                latent_z = latent_z.squeeze(1)
            # else:
            #     latent_z = self.reparameterize(prior_mu, prior_logvar, nsamples=1)
            #     latent_z = latent_z.squeeze(1)
            ...
            # Decoding
            # print(condition_pooled_hidden_fea.shape, latent_z.shape)
            ...
            # NSP_outputs = self.NSP_model(NSP_encoder, attention_mask=mask_tensor, token_type_ids=segments_tensor, next_sentence_label=label)

            # if self.args.length_weighted_loss:
            #     if int(label.item()) == 1:
            #         loss = loss_rec / sent_length + self.args.beta * loss_kl + NSP_loss
            #     else:
            #         loss = loss_rec / sent_length - 0.2 * self.args.beta * loss_kl + NSP_loss
            # else:
            #     if int(label.item()) == 1:
            #         loss = loss_rec + self.args.beta * loss_kl + NSP_loss
            #     else:
            #         loss = loss_rec - 0.05 * self.args.beta * loss_kl + NSP_loss
            if label != None:
                loss_rec = outputs[0]  # model outputs are always tuple in pytorch-transformers (see doc)
                if int(label.item()) == 1:
                    return loss_rec / sent_length, loss_kl, NSP_loss
                else:
                    return loss_rec / sent_length, 0, NSP_loss
            else:
                return outputs

        elif self.args.fb_mode==3:
            """
            有NSP，但为负样本的时候只有NSP loss
            """
        ...
        # if int(label.item()) == 1:
            ...
            return loss_rec / sent_length, loss_kl

    def NSP_predict(self, NSP_encoder, mask_tensor, segments_tensor):
        bert_outputs = self.encoder(NSP_encoder, mask_tensor, segments_tensor)
        bert_pooled_hidden_fea = bert_outputs[1]
    ...
    def log_probability(self, x, z):
        ...
        """
        outputs = self.decoder(input_ids=x, past=z, labels=x)
        loss_rec = outputs[0]
        return -loss_rec



    def loss_iw(self, x0, x1, nsamples=2000, ns=1):
        """
        Args:
        ...
        for _ in range(int(nsamples / ns)):
            ...
            ll_tmp.append(log_gen + log_prior - log_infer)  # ELBO
