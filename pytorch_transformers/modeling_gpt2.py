...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.linear = nn.Linear(self.latent_size + config.hidden_size, config.hidden_size * config.n_layer, bias=False) # different latent vector for each layer
        self.linear_emb = nn.Linear(self.latent_size + config.hidden_size, config.hidden_size, bias=False) # share the same latent vector as the embeddings
...
class GPT2ForLatentConnector(GPT2PreTrainedModel):
    ...
    def __init__(self, config, latent_size=32, latent_as_gpt_emb=True, latent_as_gpt_memory=True):
        ...
        self.transformer = GPT2Model(config)
        self.lm_head = nn.Linear(config.n_embd, 50259, bias=False)
        self.init_weights()
        self.tie_weights()
    ...
    def forward(self, input_ids, past=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        if labels is not None:
            ...
            shift_labels = labels[..., 1:].contiguous()
            # Flatten the tokens
            loss_fct = CrossEntropyLoss(ignore_index=-1, reduce=False) # 50258 is the padding id, otherwise -1 is used for masked LM.
            loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)),
                            shift_labels.view(-1))
            loss = torch.sum(loss.view(-1, shift_labels.shape[-1]), -1)
            outputs = (loss,) + outputs
